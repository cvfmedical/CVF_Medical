...
const COLUNAS_FILTRAVEIS = ['nome', 'cargo', 'nivel_acesso', 'email', 'status_ativo', 'auth_user_id'];
...
export function ConfiguracoesUsuarios() {
  ...
  const linhasFiltradas = (query.data ?? []).filter((f) =>
    COLUNAS_FILTRAVEIS.every((chave) => passaFiltro(valorColuna(f, chave), chave)),
  );
          ...
          <tr>
            {COLUNAS_FILTRAVEIS.map((chave) => {
              const valoresDisponiveis = Array.from(
                new Set((query.data ?? []).map((f) => String(valorColuna(f, chave) ?? ''))),
